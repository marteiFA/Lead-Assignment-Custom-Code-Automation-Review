...
(async function (contact, context) {
  ...
  //     };
    ...
    const clientlanguage = contact.languageS.values
    const clientContactMethod = contact.initialContactMethodRequested.value
    // const defaultLanguage = 'English'
    // const defualtContactMethod = 'Call'

    const representative = currentStateOfEmployees.filter((rep) => {return rep.leadType.display_value} )

    // return {representative: representative}

    const employeeLanguageList = representative.filter((employee) =>
        (employee.languageSkills.value || []).some(lang => {return (clientlanguage.includes(lang))})
        // (employee.languageSkills.display_value || []).some(lang => {return (clientlanguage.includes(lang) || defaultLanguage)})
      );
      
    //   return{
    //     employeeLanguageList: employeeLanguageList
    //   }

    // const representative = employeeLanguageList.filter((rep) => 
    // (rep.leadType.display_value))

    //   return{
    //     representative:representative }

    const fil = employeeLanguageList.filter(employee => {return employee.leadType.display_value === clientContactMethod})
    ...
    if ( fil.length <= 0){
      ...
      random_rep = employeeLanguageList[Math.floor(Math.random() * employeeLanguageList.length)]
    }
    // return {random_rep:random_rep}

    // return {currentStateOfEmployees:currentStateOfEmployees ,employeeLanguageList:employeeLanguageList,represent:represent ,fil:fil, random_rep:random_rep  }
    ...
    // let employee = currentStateOfEmployees.filter(
    //     (employee) =>
